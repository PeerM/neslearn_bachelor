...
from hsa.simple_dqn.state_buffer import StateBuffer
...
class Agent:
    ...
    def _restartRandom(self):
        ...
        for i in range(random.randint(self.history_length, self.random_starts) + 1):
            reward = self.env.act(random.randint(0, 255))
    ...
    def play(self, length_seconds):
        # just make sure there is history_length screens to form a state
        self._restartRandom()
        for i in range(60 * length_seconds):
